.//
.// This code snippet creates a Rockwell PLC data structure for a system
.// in the model specified by s_sys.  It is used by direct inclusion in the
.// StructuredText_SysStruct.arc files.
.//
.//
.//
.function str_CreateComponentMember  // string
  .param inst_ref pe_pe_subType
  .param string componentName
  .param string componentType
  .//
  .invoke r = createQualifiedName( pe_pe_subType )
  .assign qName = r.result
  .//
  .if (componentName == "")
    .assign componentName = "todo_add_comp_ref_classifier_name"
  .end if
  .//
  .assign attr_result = "<Member Name=""$l_{componentName}"" DataType=""${componentType}"" Dimension=""0"" Radix=""NullType"" Hidden=""false"" ExternalAccess=""Read/Write"">\n<Description>\n<![CDATA[Qualified Name: '${qName}${componentName}'\n${pe_pe_subType.Descrip}]]>\n</Description>\n</Member>"
.end function
.//
.//
.//
.function str_MakeComponentStructure_c_c  // string, string
  .param inst_ref c_c
  .//
  .assign c_c_members = ""
  .assign output = ""
...
.//
.//
.function str_MakeComponentStructure_ep_pkg  // string, string
...
.//
.// Main Code
.invoke r = str_MakeComponentStructure_ep_pkg(ep_pkg)
.if (r.output != "")
${r.output}
.end if
.if (r.result != "")
