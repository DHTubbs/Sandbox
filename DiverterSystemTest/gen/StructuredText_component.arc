...
RET( ${returnStr} );
  ...
  .if (not_empty o_cls_sm)
    .assign selfIdentifier = "cls_$l_{classIdentifier}"
    .assign eventStruct = "${selfIdentifier}.events"
...
Description: Assigner (class) state machine for class ${classIdentifier}

*)

SBR( ${selfIdentifier} );

    .select one o_sm related by o_cls_sm->SM_SM[R517]
...
end_while;
    .//  
    .emit to file "../../src/_${uniqueId}_asm$r{o_obj.Name}.txt"
    .//  
  .end if
.end for
